...
def center_of_mass_position(m: int, n: int) -> Tuple:
    ...
    com_x = np.mean(np.linspace(-Length/2 + Length/(2*m), Length/2 - Length/(2*m), m)) + L/2

    com_y = np.mean(np.linspace(-W/2 + W/(2*n), W/2 - W/(2*n), n))

    com_r = np.sqrt((com_x/L) ** 2 + (com_y/L)**2)
